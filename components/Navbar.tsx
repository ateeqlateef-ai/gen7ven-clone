...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 ${isScrolled ? 'bg-slate-950/80 backdrop-blur-xl border-b border-slate-800 py-3 shadow-2xl' : 'bg-transparent py-8'}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex space-x-10 items-center">
            ...
            <a href="contact.html" className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-black rounded-full transition-all shadow-xl uppercase tracking-widest hover:scale-105 active:scale-95">
              Start Project
            </a>
          </div>
          ...
          <div className="md:hidden flex items-center relative z-[101]">
            <button 
              ...
              className="text-slate-200 hover:text-blue-500 transition-all p-2 bg-slate-900/50 border border-slate-800 rounded-lg"
        ...
        <div className="flex flex-col space-y-10 items-center text-center">
          {links.map((link, idx) => (
            <a
              key={link.name}
              href={link.href}
              className={`text-4xl font-black transition-all tracking-tighter ${isActive(link.href) ? 'text-blue-500' : 'text-slate-500'}`}
          ...
          <a href="contact.html" className="w-full max-w-xs py-5 bg-blue-600 text-white font-black rounded-2xl text-center text-xl shadow-2xl mt-8" onClick={() => setIsMenuOpen(false)}>
            Start Project
          </a>
        </div>
      </div>
